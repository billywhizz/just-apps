...
void just::memory::Send(const FunctionCallbackInfo<Value> &args) {
  just::memory::rawBuffer* b = just::memory::buffers[Local<Integer>::Cast(args[1])->Value()];
  args.GetReturnValue().Set(Integer::New(args.GetIsolate(), send(Local<Integer>::Cast(args[0])->Value(), b->data, b->len, MSG_NOSIGNAL)));
}
